...
def exclude_eye(
	...
	eye_dict : dict
		Eye movement detection parameters. Supported keys:
			- 'window_oi' : tuple, time window to analyze (default: full epoch)
			- 'eye_ch' : str, EOG channel name (default: 'HEOG')
			- 'angle_thresh' : float, max gaze deviation in visual degrees
			...
			- 'viewing_dist' : float, viewing distance in cm (for tracker)
			- 'screen_res' : tuple, screen resolution (width, height) in pixels
			- 'screen_h' : float, screen height in cm
	eye : NpzFile, optional
		Eye tracker data file containing 'x', 'y', 'times', 'sfreq' arrays.
		If None, attempts to use eye channels in epochs. Default is None.
	...
	eeg_analyses.EYE : Eye tracking analysis class
	"""
...
def bin_tracker_angles(
	...
	----------
	angles : np.ndarray
		Deviation from fixation in visual degrees, shape (n_trials, n_times).
	thresh : float
	...
	eeg_analyses.EYE : Eye tracking analysis and angle computation
	"""
