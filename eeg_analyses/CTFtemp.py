...
class SpatialEM(BDM):
    ...
    def __init__(self, beh, epochs, channel_folder, decoding, nr_iter, nr_blocks, nr_bins, nr_chans, delta):
        ''' 
    ...
    def readData(self, subject_id, conditions, thresh_bin = 1, eye_window = (-0.3, 0.8)):
        ...
        if conditions == 'all':
            ...
            cnd_mask = np.array([cnd in conditions for cnd in beh['block_type']])
        ...
        cnds = beh['block_type'][cnd_mask] 
        pos_bins = beh[self.decoding][cnd_mask]
    ...
    def powerAnalysis(self, data, tois, sfreq, band, downsample, evoked = True):
        ...
        #OLD CODE
        E = np.empty((data.shape[0], data.shape[1], data.shape[2]), dtype=np.complex_) * np.nan; 
        T = np.copy(E)

        # Filter each electrode seperately
        for ch in range(data.shape[1]): 
            print(ch)
            E[:,ch,:] = hilbert(filter_data(data[:,ch,:], sfreq,band[0], band[1], method = 'iir', iir_params = dict(ftype = 'butterworth', order = 5)))
            T[:,ch,:] = np.abs(hilbert(filter_data(data[:,ch,:], sfreq,band[0], band[1], method = 'iir', iir_params = dict(ftype = 'butterworth', order = 5))))**2
    ...
    def crosstrainCTF(self, sj, window, train_cnds, test_cnds, freqs = dict(alpha = [8,12]), filt_art = 0.5, downsample = 4, tgm = False, name = ''):
        ...
        for test_cnd in test_cnds:
            ...
            tf = np.empty((nr_freqs, int(nr_samples), nr_te_samples, self.nr_chans)) * np.nan
            C2 = np.empty((nr_freqs, int(nr_samples), nr_te_samples, self.nr_bins, self.nr_chans)) * np.nan
            W = np.empty((nr_freqs, int(nr_samples), nr_te_samples, self.nr_chans, eegs.shape[1])) * np.nan
            ...
            for fr in range(nr_freqs):
                ...
                for tr_smpl in range(int(nr_samples)):
                ...
                # calculate slopes
                slopes = np.zeros((nr_freqs, int(nr_samples), nr_te_samples))
        ...
        with open(self.folder_tracker(['ctf',self.channel_folder,self.decoding], filename = '{}_cross-training_{}.pickle'.format(sj, name)),'wb') as handle:
    ...
    def calculateSlopes(self, data, nr_freqs, nr_samps):
        ...
        slopes = np.empty((nr_freqs, int(nr_samps))) * np.nan

        for f in range(nr_freqs):
            for s in range(int(nr_samps)):
                d = data[f,s,:] 
