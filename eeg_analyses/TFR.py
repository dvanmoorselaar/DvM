...
class TFR(FolderStructure):
	...
	def select_tf_data(self,elec_oi:Union[str,list],excl_factor:dict=None, 
					topo_flip:dict=None) -> Tuple[mne.Epochs,pd.DataFrame]:
		"""
		Selects tf data and applies inital data transformation steps in 
		the following order (all steps are optional):
		
		1. Exclude trials based on behavioral data
		2. Apply laplacian filter
		3. Flip topography
		4. Select electrodes of interest
		
		Args:
			elec_oi (Union[str,list]): Electrodes of interest. 
			Can be specified as a list of electrode names or as a string
			referring to a subset of electrodes (e.g., 'all', 'frontal')
			excl_factor (dict): Dictionary with behavioral factors to be
			excluded as keys and the values to be excluded as values.
			For example, {'cnd': [1,2]} would exclude trials with
			condition 1 and 2. Defaults to None.
			topo_flip (dict): Dictionary with behavioral factors as keys
			and the corresponding topography as values. For example,
			{'cnd': 'left'} would flip the topography of all trials
			with condition 'left'. Defaults to None.
	...
	def create_morlet(self, min_freq: int, max_freq: int, num_frex: int, 
		...
		"""
		Creates Morlet wavelets for TF decomposition 
		...
		Args:
			...
			num_frex (int): number of frequencies in between lower 
			and upperbound
			...
			nr_time (int): wavelets should be long enough such that the 
			lowest frequency wavelets tapers to zero. As a general rule, 
			nr_time can be equivalent to nr of timepoints in epoched 
			data
		...
		Returns:
			wavelets (np.array): Morlet wavelets per frequency 
			(frequencies are stored within frex)
	...
	def tf_loop(self,epochs: mne.Epochs) -> np.array:
		"""
	...
	def lateralized_tf(self,pos_labels:dict,cnds:dict=None, 
					   ...
					   topo_flip:dict=None,window_oi:tuple=None, 
					   excl_factor:dict=None,name:str='main'):
		
		# get data
		epochs, beh = self.select_tf_data(elec_oi,excl_factor,topo_flip)
		...
		tf = {'ch_names':np.array(epochs.ch_names), 
		...
		for c, cnd in enumerate(cnds):
			...
			# TF decomposition 
			raw_conv = self.tf_loop(epochs[idx_c])
		...
		# save output
		self.save_to_mne_format(tf,epochs,tf_name)
