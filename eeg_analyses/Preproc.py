...
def preproc_eeg(sj: int, session: int, eeg_runs: list, nr_sessions: int, eog: list, ref: list, t_min: float, 
    ...
    EEG = mne.concatenate_raws([RawBDF(FS.FolderTracker(extension=['raw'], 
    ...
    missing, report_str = epochs.align_behavior(events, trigger_header = trigger_header, headers = project_param, bdf_remove = bdf_remove)
