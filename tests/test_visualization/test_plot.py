...
class TestPlotSignificance:
    ...
    def test_2d_without_yval_raises(self):
        ...
        with pytest.raises(ValueError, match="y_val"):
            plot_significance(
                np.arange(20), y, stats="ttest", sig_mask=np.ones((5, 20), dtype=bool)
            )
    ...
    def test_2d_perm_draws_contour_per_cluster(self):
        ...
        plot_significance(np.arange(20), y, stats="perm", y_val=np.arange(5), sig_mask=[cluster])
    ...
    def test_smooth_applies_gaussian_filter_1d(self):
        ...
        plt.plot(x, y.mean(axis=0), color="green")
        plot_significance(x, y, stats="ttest", sig_mask=sig_mask, color="green", smooth=True)
    ...
    def test_smooth_2d_perm_no_crash(self):
        ...
        plt.imshow(y.mean(axis=0), aspect="auto", origin="lower")
        plot_significance(
            np.arange(20), y, stats="perm", y_val=np.arange(5), sig_mask=[cluster], smooth=True
        )
    ...
    def test_smooth_2d_ttest_no_crash(self):
        ...
        plt.imshow(y.mean(axis=0), aspect="auto", origin="lower")
        plot_significance(
            np.arange(20), y, stats="ttest", y_val=np.arange(5), sig_mask=sig_mask, smooth=True
        )
...
class TestPlotSignificanceConditionDiff:
    ...
    def test_y2_tests_paired_difference_not_y_alone(self):
        ...
        plot_significance(np.arange(40), y, y2=y2, stats="ttest", color="blue", bar_y=-5.0)
    ...
    def test_bar_y_draws_markers_at_fixed_height_not_on_waveform(self):
        ...
        plot_significance(x, y, stats="ttest", sig_mask=sig_mask, color="grey", bar_y=-2.5)
    ...
    def test_bar_y_uses_scatter_not_line(self):
        ...
        plot_significance(x, y, stats="perm", sig_mask=sig_mask, color="grey", bar_y=-1.0)
    ...
    def test_point_colors_overrides_flat_color_per_timepoint(self):
        ...
        sig_mask = np.zeros(20, dtype=bool)
        sig_mask[2:8] = True
        point_colors = np.array(["red" if i < 5 else "green" for i in range(20)])
    ...
    def test_point_colors_none_falls_back_to_flat_color(self):
        ...
        plot_significance(x, y, stats="ttest", sig_mask=sig_mask, color="red", bar_y=0.0)
    ...
    def test_marker_size_controls_scatter_s(self):
        ...
        plot_significance(
            x, y, stats="ttest", sig_mask=sig_mask, color="grey", bar_y=0.0, marker_size=77
        )
...
class TestPlotErpTimecourse:
    ...
    def test_amplitude_converted_to_microvolts(self):
        ...
        erps = {
            "A": make_condition_evokeds({"C3": 5e-6}, ch_names=["C3"], n_subjects=3, noise_sd=0)
        }
    ...
    def test_already_microvolt_scale_not_rescaled(self):
        ...
        erps = {"A": make_condition_evokeds({"C3": 5.0}, ch_names=["C3"], n_subjects=3, noise_sd=0)}
        times = erps["A"][0].times
    ...
    def test_multiple_electrode_groups_get_contra_ipsi_labels(self):
        erps = {"A": make_condition_evokeds({"C3": 3.0, "C4": 1.0}, n_subjects=3, noise_sd=0)}
        times = erps["A"][0].times
    ...
    def test_lateralized_computes_contra_minus_ipsi(self):
        ...
        plot_erp_timecourse(erps, times, elec_oi=[["C3"], ["C4"]], lateralized=True)
    ...
    def test_colors_list_not_mutated_across_calls(self):
        erps = {"A": make_condition_evokeds({"C3": 1.0}, ch_names=["C3"], n_subjects=3, noise_sd=0)}
        times = erps["A"][0].times
        my_colors = ["red", "blue"]
    ...
    def test_window_oi_tuple_draws_rectangle(self):
        ...
        erps = {
            "A": make_condition_evokeds(
                {"C3": 1.0}, ch_names=["C3"], n_subjects=3, noise_sd=0, sfreq=200, n_samples=60
            )
        }
    ...
    def test_stats_overlay_draws_extra_line(self):
        erps = {
            "A": make_condition_evokeds(
                {"C3": 5.0}, ch_names=["C3"], n_subjects=15, noise_sd=0.2, seed=0
            ),
        }
        times = erps["A"][0].times
    ...
    def test_list_input_treated_as_single_unnamed_condition(self):
        evokeds = make_condition_evokeds({"C3": 1.0}, ch_names=["C3"], n_subjects=3, noise_sd=0)
    ...
    def test_cnds_filters_to_requested_conditions(self):
        erps = {
            ...
            "B": make_condition_evokeds(
                {"C3": 2.0}, ch_names=["C3"], n_subjects=3, noise_sd=0, seed=1
            ),
        }
        times = erps["A"][0].times
    ...
    def test_cnd_diff_marks_significant_window_and_auto_colors(self):
        ...
        expected = np.array([red if i % 2 == 0 else green for i in range(facecolors.shape[0])])
        np.testing.assert_allclose(facecolors, expected)
    ...
    def test_cnd_diff_requires_stats(self):
        erps = {
            "A": make_condition_evokeds({"C3": 1.0}, ch_names=["C3"], n_subjects=5, noise_sd=0.1),
            "B": make_condition_evokeds(
                {"C3": 2.0}, ch_names=["C3"], n_subjects=5, noise_sd=0.1, seed=1
            ),
        }
        times = erps["A"][0].times
        ...
        with pytest.raises(ValueError, match="stats"):
            plot_erp_timecourse(
                erps, times, elec_oi=["C3"], cnds=["A", "B"], stats=False, cnd_diff=("A", "B")
            )
...
class TestPlotTfrTimecourse:
    ...
    def test_1d_freq_band_matches_manual_average(self):
        tfr = make_average_tfr(
            ch_names=["C3"], freqs=np.linspace(4, 20, 8), amplitude_by_channel={"C3": 2.0}
        )
        tfr_dict = {"A": tfr}
    ...
    def test_elec_oi_all_averages_every_channel(self):
        ...
        plot_tfr_timecourse(tfr_dict, elec_oi="all", timecourse="1d", freq_oi=(8, 12), cnds=["A"])
    ...
    def test_onset_marker_drawn_in_1d_mode(self):
        ...
        plot_tfr_timecourse(
            tfr_dict, elec_oi=["C3"], timecourse="1d", freq_oi=(8, 12), cnds=["A"], onset_times=[0]
        )
    ...
    def test_colors_list_not_mutated_across_conditions(self):
        tfr_dict = {
            "A": make_average_tfr(
                ch_names=["C3", "C4"], amplitude_by_channel={"C3": 1.0, "C4": 1.0}
            ),
            "B": make_average_tfr(
                ch_names=["C3", "C4"], amplitude_by_channel={"C3": 1.0, "C4": 1.0}
            ),
    ...
    def test_2d_contour_and_heatmap_share_data_range(self):
        ...
        plot_tfr_timecourse(tfr_dict, elec_oi=["C3"], timecourse="2d", cnds=["A"], contour=False)
        heatmap_data = plt.gca().get_images()[0].get_array()
        plt.close("all")

        plot_tfr_timecourse(tfr_dict, elec_oi=["C3"], timecourse="2d", cnds=["A"], contour=True)
    ...
    def test_1d_single_frequency_selection(self):
        ...
        tfr_dict = {
            "A": [
                make_average_tfr(ch_names=["C3"], freqs=freqs, amplitude_by_channel={"C3": 3.0})
                for _ in range(3)
            ]
        }

        plot_tfr_timecourse(tfr_dict, elec_oi=["C3"], timecourse="1d", freq_oi=8, cnds=["A"])
    ...
    def test_1d_stats_overlay_draws_extra_line(self):
        ...
        tfrs = []
        for _ in range(15):
            tfr = make_average_tfr(ch_names=["C3"], freqs=freqs, amplitude_by_channel={"C3": 3.0})
            tfr.data += rng.normal(0, 0.2, tfr.data.shape)
            tfrs.append(tfr)

        plot_tfr_timecourse(
            {"A": tfrs}, elec_oi=["C3"], timecourse="1d", freq_oi=(8, 12), cnds=["A"], stats="ttest"
        )
...
class TestPlotBdmTimecourse:
    ...
    def test_2d_tfr_uses_frequency_axis(self):
        ...
        bdms = make_bdm_result(np.full((6, 10), 0.6), times=times, freqs=freqs, noise_sd=0)
    ...
    def test_ttest_stats_2d_gat_no_crash(self):
        ...
        bdms = [
            {
                "A": {"dec_scores": base + rng.normal(0, 0.3, (15, 15))},
    ...
    def test_multi_condition_2d_mode_plots_first_only(self):
        ...
        bdms = make_bdm_result(
            np.full((10, 10), 0.6), times=times, test_times=times, cnd="A", noise_sd=0
        )
        for b in bdms:
    ...
    def test_mask_nonsig_draws_greyscale_then_color_overlay(self):
        ...
        plot_bdm_timecourse(bdms, timecourse="2d_GAT", stats="ttest", mask_nonsig=True)

        # greyscale background image + significant-overlay image
        assert len(plt.gca().get_images()) == 2

    @staticmethod
    def _make_two_cnd_bdms(n_sub=15, n_t=40, seed_easy=1, seed_hard=2, easy_val=0.9, hard_val=0.55):
        ...
        bdms = make_bdm_result(
            easy_scores, times=times, cnd="easy", n_subjects=n_sub, noise_sd=0.05, seed=seed_easy
        )
        hard_bdms = make_bdm_result(
            hard_scores, times=times, cnd="hard", n_subjects=n_sub, noise_sd=0.05, seed=seed_hard
        )
    ...
    def test_cnd_diff_multi_contrast_stacks_into_separate_rows(self):
        ...
        medium_bdms = make_bdm_result(
            medium_scores, times=times, cnd="medium", n_subjects=15, noise_sd=0.05, seed=3
        )
    ...
    def test_cnd_diff_2d_gat_draws_extra_contour_no_crash(self):
        bdms = self._make_two_cnd_bdms()
        for b in bdms:
            b["easy"]["dec_scores"] = np.outer(b["easy"]["dec_scores"], b["easy"]["dec_scores"])
            b["hard"]["dec_scores"] = np.outer(b["hard"]["dec_scores"], b["hard"]["dec_scores"])
            b["info"]["test_times"] = b["info"]["times"]

        plot_bdm_timecourse(
            bdms, timecourse="2d_GAT", cnds=["easy"], stats="perm", cnd_diff=("easy", "hard")
        )
    ...
    def test_cnd_diff_missing_condition_raises(self):
        ...
        with pytest.raises(ValueError, match="not found"):
            plot_bdm_timecourse(
                bdms, cnds=["easy", "hard"], stats="perm", cnd_diff=("easy", "nonexistent")
            )
    ...
    def test_cnd_diff_requires_stats(self):
        ...
        with pytest.raises(ValueError, match="stats"):
            plot_bdm_timecourse(bdms, cnds=["easy", "hard"], stats=False, cnd_diff=("easy", "hard"))
...
class TestPlotCtfTimecourse:
    ...
    def test_1d_multi_band_no_band_oi_warns_and_averages(self):
        ...
        with pytest.warns(UserWarning, match="band_oi"):
            plot_ctf_timecourse(
                ctfs, timecourse="1d", output="raw_slopes", band_oi=None, stats=False
            )
    ...
    def test_band_oi_selects_specific_band(self):
        ...
        plot_ctf_timecourse(
            ctfs, timecourse="1d", output="raw_slopes", band_oi="alpha", stats=False
        )
    ...
    def test_ttest_stats_2d_gat_no_crash(self):
        ...
        plot_ctf_timecourse(ctfs, timecourse="2d_gat", output="raw_slopes", stats="ttest")
    ...
    def test_multi_condition_2d_mode_plots_first_only(self):
        ...
        ctfs = make_ctf_result(
            np.full((1, 10), 0.3), times=times, bands=["all"], cnd="A", noise_sd=0
        )
        for c in ctfs:
            c["B"] = {"raw_slopes": np.full((1, 10), 0.9)}

        plot_ctf_timecourse(
            ctfs, timecourse="2d_gat", output="raw_slopes", cnds=["A", "B"], stats=False
        )
    ...
    def test_multiple_outputs_get_combined_labels(self):
        ...
        for _ in range(3):
            ctfs.append(
                {
                    "A": {
                        "raw_slopes": np.full((1, 10), 0.2) + rng.normal(0, 0.01, (1, 10)),
        ...
        plot_ctf_timecourse(
            ctfs, timecourse="1d", output=["raw_slopes", "param_slopes"], stats=False
        )
    ...
    def test_avg_bins_averages_only_nonzero_bins(self):
        ...
        plot_ctf_timecourse(
            ctfs, timecourse="1d", output="raw_slopes", plot_bins=False, stats=False
        )
    ...
    def test_1d_single_band_stats_overlay_draws_extra_line(self):
        times = np.linspace(-0.1, 0.5, 15)
        raw = np.full((1, 15), 0.3)
        ctfs = make_ctf_result(raw, times=times, bands=["all"], n_subjects=15, noise_sd=0.2, seed=2)

        plot_ctf_timecourse(ctfs, timecourse="1d", output="raw_slopes", stats="ttest")
    ...
    def test_2d_tfr_uses_frequency_axis(self):
        ...
        plot_ctf_timecourse(ctfs, timecourse="2d_tfr", output="raw_slopes", stats=False)
    ...
    def test_band_oi_without_bands_key_raises(self):
        times = np.linspace(-0.1, 0.5, 10)
        ctf = {"A": {"raw_slopes": np.full((1, 10), 0.3)}, "info": {"times": times}}

        with pytest.raises(ValueError, match="'bands' key not found"):
            plot_ctf_timecourse(ctf, output="raw_slopes", band_oi="alpha", stats=False)
    ...
    def test_2d_tfr_without_freqs_key_raises(self):
        ...
        with pytest.raises(ValueError, match="'freqs' key not found"):
            plot_ctf_timecourse(ctf, timecourse="2d_tfr", output="raw_slopes", stats=False)
    ...
    def test_stats_with_individual_bins_warns_and_skips(self):
        ...
        ctfs = make_ctf_result(raw, times=times, bands=["all"], n_subjects=5, noise_sd=0.1, seed=1)
    ...
    def _make_two_cnd_ctfs(n_sub=15, n_t=40):
        ...
        ctfs = make_ctf_result(
            easy, times=times, bands=["all"], cnd="easy", n_subjects=n_sub, noise_sd=0.3, seed=1
        )
        hard_ctfs = make_ctf_result(
            hard, times=times, bands=["all"], cnd="hard", n_subjects=n_sub, noise_sd=0.3, seed=2
        )
    ...
    def test_cnd_diff_2d_gat_draws_extra_contour_no_crash(self):
        ...
        ctfs = make_ctf_result(
            easy_gat, times=times, bands=["all"], cnd="easy", n_subjects=15, noise_sd=0.3, seed=1
        )
        hard_ctfs = make_ctf_result(
            hard_gat, times=times, bands=["all"], cnd="hard", n_subjects=15, noise_sd=0.3, seed=2
        )
        for c, hc in zip(ctfs, hard_ctfs):
            c["hard"] = hc["hard"]
    ...
    def test_cnd_diff_with_individual_bins_warns_and_skips(self):
        ...
        ctfs = make_ctf_result(
            raw_easy, times=times, bands=["all"], cnd="easy", n_subjects=5, noise_sd=0.1, seed=1
        )
        hard_ctfs = make_ctf_result(
            raw_hard, times=times, bands=["all"], cnd="hard", n_subjects=5, noise_sd=0.1, seed=2
        )
        for c, hc in zip(ctfs, hard_ctfs):
            c["hard"] = hc["hard"]
...
class TestPlotErpTopography:
    ...
    def test_window_oi_restricts_averaging_window(self):
        ch_names = ["Cz"]
        info = mne.create_info(ch_names, 100.0, ch_types="eeg")
        ...
        with patch.object(plotmod, "plot_topography", side_effect=fake_plot_topography):
            plot_erp_topography(
                erps,
                times,
                montage=None if False else "biosemi64",
                topo="raw",
                window_oi=(0.0, 0.1),
            )
