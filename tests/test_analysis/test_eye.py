...
class TestEyeInit:
    ...
    def test_custom_attributes(self):
        eye_obj = EYE(viewing_dist=90, screen_res=(3840, 2160), screen_h=35, sfreq=1000)
...
class TestGetEyeData:
    ...
    def test_glob_discovery_via_all(self, tmp_path, monkeypatch):
        ...
        write_asc_session(tmp_path / "eye" / "raw" / "sub_01_ses_1.asc", n_trials=2)
        write_beh_csv(tmp_path / "behavioral" / "raw" / "sub_01_ses_1.csv", n_trials=2)
    ...
    @pytest.mark.unit
    def test_multi_session_glob_missing_eye_file_drops_matching_beh(self, tmp_path, monkeypatch):
        """
        ...
        for ses in (1, 3, 5):
            write_asc_session(tmp_path / "eye" / "raw" / f"sub_01_ses_{ses}.asc", n_trials=1)
    ...
    @pytest.mark.unit
    def test_more_beh_than_eye_trials_removes_extra_beh_rows(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_asc_session("sample.asc", n_trials=2)
    ...
    def test_practice_trials_removed(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_asc_session("sample.asc", n_trials=2)
        write_beh_csv("sample.csv", n_trials=2, extra_cols={"practice": ["yes", "no"]})
    ...
    @pytest.mark.unit
    def test_tsv_files_do_not_crash_on_missing_trial_info(self, tmp_path, monkeypatch):
        """
    ...
    @pytest.mark.unit
    def test_multi_file_trial_info_correctly_flattened(self, tmp_path, monkeypatch):
        """
    ...
    @pytest.mark.unit
    def test_partial_nan_trial_info_interpolated_from_neighbors(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        lines = [
            "MSG\t1000 start trial: 1\n",
            "MSG\t1005 TRIALID 10\n",
    ...
    @pytest.mark.unit
    def test_eye_trials_mismatch_uses_nr_trials_mask(self, tmp_path, monkeypatch):
        """
        ...
        write_beh_csv("sample.csv", n_trials=1, extra_cols={"nr_trials": [1, 2, 99]})
        ...
        def fake_read_edf(file, start, trial_info=None, missing=0):
            trial0 = make_trial(
                ...
                y=[400.0],
                trackertime=[1000],
                msgs=[[1000, "start trial: 1"], [1002, "start trial: 1 (dup)"]],
            )
            trial1 = make_trial(
                x=[500.0],
                y=[400.0],
    ...
    @pytest.mark.unit
    def test_no_behavioral_files_returns_index_only_df(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_asc_session("sample.asc", n_trials=2)
...
class TestInterpTrial:
    ...
    def test_blink_linearly_interpolated(self):
        ...
        y[200:206] = 0.0
        x[-1], y[-1] = 120.0, 220.0
        trial = make_trial(x=x, y=y, trackertime=np.arange(n), blinks=[(200, 205)])
    ...
    def test_blink_near_trial_start_now_interpolated(self):
        ...
        x[5:11] = 0.0
        y[5:11] = 0.0
        trial = make_trial(x=x, y=y, trackertime=np.arange(n), blinks=[(5, 10)])
    ...
    def test_blink_padding_covers_whole_short_trial_returns_zeros(self):
        ...
        x[40:60] = 0.0
        y[40:60] = 0.0
        trial = make_trial(x=x, y=y, trackertime=np.arange(n), blinks=[(40, 59)])
...
class TestGetXy:
    ...
    def test_basic_extraction_aligned_to_event(self):
        ...
        x_out, y_out, times = eye_obj.get_xy(eye, -20, 20, "trigger", interpolate_blinks=False)
    ...
    def test_sparse_data_no_longer_crashes(self):
        ...
        x_out, y_out, times = eye_obj.get_xy(eye, -10, 10, "trigger", interpolate_blinks=False)
    ...
    def test_interpolate_blinks_requires_full_pre_window_coverage(self):
        ...
        blink_rel = np.where((times >= blink_s - trigger_t) & (times <= blink_e - trigger_t))[0]
        ...
        x_partial, _, times2 = eye_obj.get_xy(eye_partial, -500, 500, "trigger")
        blink_rel2 = np.where((times2 >= blink_s - trigger_t) & (times2 <= blink_e - trigger_t))[0]
...
class TestSetXy:
    ...
    def test_globally_zero_edge_columns_excluded_from_processing(self):
        eye_obj = EYE(sfreq=1000, screen_res=(1000, 800), viewing_dist=60, screen_h=30)
        n = 400
        ...
        x_out, y_out = eye_obj.set_xy(x.copy(), y.copy(), times, drift_correct=None)
    ...
    def test_drift_correction_recenters_stable_fixation(self):
        ...
        x_out, y_out = eye_obj.set_xy(x.copy(), y.copy(), times, drift_correct=(-100, 0))
        np.testing.assert_allclose(x_out, 500.0)
...
class TestAnglesFromXyAndBins:
    ...
    def test_create_angle_bins_sustained_deviation(self):
        eye_obj = EYE(sfreq=1000, screen_res=(1920, 1080), viewing_dist=60, screen_h=30)
        n = 100
        x = np.full(n, 960.0)
        y = np.full(n, 540.0)
    ...
    def test_angles_from_xy_matches_create_angle_bins_hardcoded_params(self):
        ...
        _, expected_angles = eye_obj.create_angle_bins(x, y, 0, 3, 0.25, 40)
    ...
    def test_angles_from_xy_applies_drift_correct_first(self):
        ...
        angles = eye_obj.angles_from_xy(x.copy(), y.copy(), times, drift_correct=(-100, 0))
...
class TestLinkEyeToEeg:
    ...
    @pytest.mark.unit
    def test_positional_arguments_reach_get_eye_data_correctly(self, tmp_path, monkeypatch):
        """
...
class TestSaccadeDetectorInit:
    ...
    def test_attributes_and_deg_per_pixel(self):
        sd = SaccadeDetector(sfreq=1000, screen_h=30, viewing_dist=60, screen_res=(1920, 1080))
...
class TestCalcVelocity:
    ...
    def test_linear_ramp_gives_constant_velocity_zero_acceleration(self):
        ...
        expected_V = np.sqrt(vx_true**2 + vy_true**2) * sd.deg_per_pixel * sd.sfreq
    ...
    def test_quadratic_ramp_gives_constant_acceleration(self):
        ...
        V, A = sd.calc_velocity(x, y)
        expected_A = np.sqrt(ax_true**2 + ay_true**2) * sd.deg_per_pixel * sd.sfreq**2
        np.testing.assert_allclose(A[20:280], expected_A, rtol=1e-6)

        tt = 102
        vx_t, vy_t = vx0 + ax_true * tt, vy0 + ay_true * tt
        expected_V_t = np.sqrt(vx_t**2 + vy_t**2) * sd.deg_per_pixel * sd.sfreq
...
class TestNoiseDetect:
    ...
    def test_blink_marked_nan_no_extension_when_velocity_low(self):
        ...
        np.testing.assert_array_equal(np.where(np.isnan(x_out))[0], [20, 21, 22, 23, 24])
...
class TestEstimateThresh:
    ...
    def test_matches_independently_computed_center_statistics(self):
        ...
        center = V[int(math.floor(0 + cent_fix)) : int(math.ceil((n - 1) - cent_fix) + 1)]
...
class TestSaccadeDetection:
    ...
    @pytest.mark.unit
    def test_high_velocity_glissade_bump_also_skipped_as_separate_saccade(self):
        """
...
class TestDetectEvents:
    ...
    def test_missing_data_trial_preserved_as_nan_not_cast_to_true(self):
        ...
        sd = SaccadeDetector(sfreq=1000, screen_h=30, viewing_dist=60, screen_res=(1920, 1080))
        n = 200
        x_clean = np.full(n, 960.0) + np.random.RandomState(0).normal(0, 0.5, n)
        y_clean = np.full(n, 540.0) + np.random.RandomState(1).normal(0, 0.5, n)
