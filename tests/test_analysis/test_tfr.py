...
class TestSelectTfrData:
    ...
    def test_topo_flip_calls_flip_topography_without_crash(self):
        ...
        ep, out_df = tfr.select_tfr_data(elec_oi="all", topo_flip={"target_loc": [1]})
...
class TestCreateFreqBands:
    ...
    def test_linear_bands_match_manual_calculation(self):
        ...
        df = make_behavioral_df(3, x=1)
        tfr = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=5, freq_scaling="linear"
        )
    ...
    def test_log_bands_cover_full_range_contiguously(self):
        ...
        df = make_behavioral_df(3, x=1)
        tfr = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=5, freq_scaling="log"
        )
        bands = tfr.create_freq_bands()
...
class TestCreateMorlet:
    ...
    def test_center_frequencies_match_requested(self):
        ...
        df = make_behavioral_df(3, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df)
        wavelets, frex = tfr.create_morlet(4, 20, 5, (3, 10), "log", 200, 100.0)
        for i, f in enumerate(frex):
            peak = _fft_peak_freq(wavelets[i], 100.0)
    ...
    def test_normalize_wavelets_gives_unit_energy(self):
        ...
        df = make_behavioral_df(3, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df)
        wavelets, _ = tfr.create_morlet(
            4, 20, 5, (3, 10), "log", 200, 100.0, normalize_wavelets=True
        )
    ...
    def test_no_normalization_gives_nonunit_energy(self):
        ...
        tfr = TFR(sj=1, epochs=epochs, df=df)
        wavelets, _ = tfr.create_morlet(
            4, 20, 5, (3, 10), "log", 200, 100.0, normalize_wavelets=False
        )
        energies = np.sum(np.abs(wavelets) ** 2, axis=1)
    ...
    def test_invalid_freq_scaling_raises(self):
        ...
        with pytest.raises(ValueError):
            tfr.create_morlet(4, 20, 5, (3, 10), "bogus", 200, 100.0)
    ...
    def test_linear_scaling_center_frequencies_match_requested(self):
        ...
        df = make_behavioral_df(3, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df)
        wavelets, frex = tfr.create_morlet(4, 20, 5, (3, 10), "linear", 200, 100.0)
...
class TestTfrLoop:
    ...
    def test_wavelet_method_power_peaks_at_true_frequency(self):
        epochs = make_oscillating_epochs(
            freq=10,
            ch_names=["C3"],
            n_trials=6,
            n_samples=200,
    ...
    def test_hilbert_method_power_peaks_at_true_frequency(self):
        ...
        df = make_behavioral_df(6, x=1)
        tfr = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=30, num_frex=10, method="hilbert"
        )
        tfr._ensure_wavelets()
...
class TestComputeTfrs:
    ...
    def test_induced_power_removes_phase_locked_component(self):
        ...
        tfr_total = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=10, power="total"
        )
        X_total = tfr_total.compute_tfrs(epochs.copy(), output="power", for_decoding=True)

        tfr_induced = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=10, power="induced"
        )
        cnd_idx = [np.arange(10)]
        X_induced = tfr_induced.compute_tfrs(
            epochs.copy(), output="power", for_decoding=True, cnd_idx=cnd_idx
        )
    ...
    def test_induced_without_cnd_idx_raises(self):
        epochs = make_epochs(n_trials=4, n_samples=20)
        df = make_behavioral_df(4, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3, power="induced")
    ...
    def test_for_decoding_percent_change_baseline(self):
        epochs = make_epochs(n_trials=4, n_samples=40, sfreq=100.0)
        df = make_behavioral_df(4, x=1)
        tfr = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3, baseline=(-0.2, 0)
        )
    ...
    def test_for_decoding_no_baseline_uses_raw_power(self):
        epochs = make_epochs(n_trials=4, n_samples=20)
        df = make_behavioral_df(4, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3, baseline=None)
    ...
    def test_for_decoding_false_applies_db_conversion(self):
        ...
        tfr_raw = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3, baseline=None
        )
        X_raw = tfr_raw.compute_tfrs(epochs.copy(), output="power", for_decoding=True)

        tfr_db = TFR(
            sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3, baseline=(-0.2, 0)
        )
        X_db = tfr_db.compute_tfrs(epochs.copy(), output="power", for_decoding=False)
...
class TestBaselineTfr:
    ...
    def test_invalid_method_raises(self):
        ...
        with pytest.raises(ValueError, match="Invalid method"):
            TFR.baseline_tfr(tfr_obj, tfr, base={"condA": np.full((1, 1), 1.0)}, method="bogus")
...
class TestLateralizationIndex:
    ...
    def _make_average_tfr(ch_names, values, freqs=None, times=None):
        ...
        info = mne.create_info(list(ch_names), 100.0, ch_types="eeg")
    ...
    def test_elec_oi_all_modifies_tfr_data_in_place(self):
        ...
        np.testing.assert_allclose(result["condA"][0]._data[0, 0, 0], (10 - 2) / (10 + 2))
...
class TestConditionTfrs:
    ...
    def test_single_default_condition(self, tmp_path, monkeypatch):
        ...
        epochs = make_epochs(n_trials=6, n_samples=40, sfreq=100.0)
        df = make_behavioral_df(6, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3)
    ...
    def test_window_oi_crops_time_axis(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        epochs = make_epochs(n_trials=4, n_samples=100, sfreq=100.0, tmin=-0.5)
        df = make_behavioral_df(4, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3)
    ...
    def test_baseline_trial_spec_runs_without_crash(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        epochs = make_epochs(n_trials=6, n_samples=40, sfreq=100.0, tmin=-0.2)
        df = make_behavioral_df(6, x=1)
        tfr = TFR(
            sj=1,
            epochs=epochs,
    ...
    def test_evoked_power_via_condition_tfrs(self, tmp_path, monkeypatch):
        ...
        epochs = make_epochs(n_trials=6, n_samples=40, sfreq=100.0)
        df = make_behavioral_df(6, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=3, power="evoked")
    ...
    def test_report_true_generates_html_file(self, tmp_path, monkeypatch):
        ...
        # plots" branch too
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=5, report=True)
    ...
    def test_report_with_three_or_fewer_freqs(self, tmp_path, monkeypatch):
        ...
        epochs = mne.EpochsArray(data, info, tmin=-0.2)
        df = make_behavioral_df(6, x=1)
        tfr = TFR(sj=1, epochs=epochs, df=df, min_freq=4, max_freq=20, num_frex=2, report=True)
