...
class TestFolderTracker:
    ...
    @pytest.mark.unit
    def test_overwrite_true_returns_unchanged_path_even_if_exists(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
    ...
    @pytest.mark.unit
    def test_overwrite_false_appends_plus_before_extension(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "data.fif").write_text("x")
...
class TestReadErps:
    ...
    @pytest.mark.unit
    def test_match_true_truncates_mismatched_sample_counts(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_evoked(tmp_path, "01", "all_data", "locked", make_evoked(n_samples=10, value=1.0))
        write_evoked(tmp_path, "02", "all_data", "locked", make_evoked(n_samples=8, value=1.0))

        erps, times = FolderStructure().read_erps(erp_name="locked", sjs=[1, 2], match=True)
...
class TestReadTfr:
    ...
    def test_single_condition_single_subject(self, tmp_path, monkeypatch):
        ...
        tfr = FolderStructure().read_tfr(tfr_folder_path=["wavelet"], tfr_name="locked", sjs=[1])
    ...
    def test_multiple_conditions_and_subjects(self, tmp_path, monkeypatch):
        ...
        tfr = FolderStructure().read_tfr(
            tfr_folder_path=["wavelet"], tfr_name="locked", cnds=["left", "right"], sjs=[1, 2]
        )
    ...
    def test_sjs_all_globs_every_subject(self, tmp_path, monkeypatch):
        ...
        tfr = FolderStructure().read_tfr(tfr_folder_path=["wavelet"], tfr_name="locked", sjs="all")
...
class TestReadBdm:
    ...
    @pytest.mark.unit
    def test_multiple_analyses_prefixed_with_names_when_no_labels(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_bdm_pickle(
            tmp_path,
            ["loc"],
    ...
    @pytest.mark.unit
    def test_underscore_containing_condition_names_preserved(self, tmp_path, monkeypatch):
...
class TestLoadProcessedEpochs:
    ...
    def test_uses_epochs_metadata_when_present(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_epochs(
            tmp_path, "01", "main", metadata=pd.DataFrame({"cond": ["a", "b", "a", "b", "a"]})
        )
    ...
    def test_falls_back_to_beh_csv_when_no_metadata(self, tmp_path, monkeypatch):
        ...
        write_processed_beh(tmp_path, "01", "main", pd.DataFrame({"cond": ["x"] * 5}))
    ...
    def test_excl_factor_removes_matching_trials(self, tmp_path, monkeypatch):
        ...
        write_epochs(
            ...
            "01",
            "main",
            metadata=pd.DataFrame({"cue": ["left", "right", "left", "right", "left"]}),
        )
    ...
    def test_eye_dict_missing_preproc_file_warns_and_falls_back_to_eog(
        ...
        monkeypatch.chdir(tmp_path)
        write_epochs(tmp_path, "01", "ses_1_main", metadata=pd.DataFrame({"cond": ["a"] * 5}))
    ...
    @pytest.mark.unit
    def test_eye_dict_restored_even_if_exclude_eye_raises(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.unit
    def test_eye_dict_uses_tracker_data_when_preproc_file_present(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        write_epochs(tmp_path, "01", "ses_1_main", metadata=pd.DataFrame({"cond": ["a"] * 5}))
        os.makedirs(tmp_path / "preprocessing" / "group_info", exist_ok=True)
        (tmp_path / "preprocessing" / "group_info" / "preproc_param_main.json").write_text("{}")
    ...
    def test_eye_dict_finds_combined_session_npz_file(self, tmp_path, monkeypatch):
        ...
        # (sub_01_all_main.npz).
        monkeypatch.chdir(tmp_path)
        write_epochs(tmp_path, "01", "all_main", metadata=pd.DataFrame({"cond": ["a"] * 5}))
        os.makedirs(tmp_path / "preprocessing" / "group_info", exist_ok=True)
        (tmp_path / "preprocessing" / "group_info" / "preproc_param_main.json").write_text("{}")
        os.makedirs(tmp_path / "eye" / "processed", exist_ok=True)
...
class TestRegressions:
    ...
    @pytest.mark.unit
    def test_folder_tracker_multidot_filename_uses_last_dot(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        (tmp_path / "sub_1.2_data.fif").write_text("x")

        path = FolderStructure.folder_tracker(fname="sub_1.2_data.fif", overwrite=False)
