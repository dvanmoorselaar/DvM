...
class TestReadEdf:
    ...
    def test_splits_into_two_trials(self, tmp_path):
        ...
        data, trial_nr = read_edf(str(path), start='start_trial',
                                   trial_info='TRIALID', missing=0.0)
    ...
    def test_gaze_samples_and_missing_sample_zeroed(self, tmp_path):
        ...
        np.testing.assert_array_equal(data[0]['trackertime'],
                                       [1001, 1002, 1011])
    ...
    def test_explicit_stop_marker(self, tmp_path):
        ...
        data, _ = read_edf(str(path), start='start_trial',
                            stop='stop_trial', missing=0.0)
...
class TestReadEdfTimeOverlap:
    ...
    def test_missing_file_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            read_edf_time_overlap(str(tmp_path / 'nope.asc'),
...
class TestReadEyetribe:
    ...
    def test_gaze_samples_captured(self, tmp_path):
        ...
        np.testing.assert_array_equal(data[0]['x'][:5], [500., 501., 502., 503., 504.])
    ...
    def test_explicit_stop_marker(self, tmp_path):
        ...
        data = read_eyetribe(str(path), start='start_trial',
                              stop='stop_trial', missing=0.0)

        assert len(data) == 2
        np.testing.assert_array_equal(data[0]['x'], [500., 501., 502.])
        np.testing.assert_array_equal(data[1]['x'], [600., 601., 602.])
