...
def bootstrap_SE(X: np.ndarray, nr_iter: int = 9999) -> Tuple[np.ndarray, np.ndarray]:
...
def confidence_int(
    data: np.ndarray, p_value: float = 0.05, tail: str = "two", morey: bool = True
) -> np.ndarray:
...
def connected_adjacency(
    image: np.ndarray, connect: str, patch_size: Tuple[int, int] = (1, 1)
) -> sparse.csr_matrix:
    ...
    if connect == "4":
        ...
        raise ValueError(f'Invalid parameter \'connect\'={connect!r}, must be "4" or "8".')
...
def perform_stats(
    ...
    **Important - Statistical control differences**: The three tests use
    ...
    if stat_test == "perm":
        ...
        raise ValueError(f"stat_test must be 'perm', 'ttest', or 'fdr', got '{stat_test}'")
