...
class ERP(FolderStructure):
    ...
    def select_erp_data(
        self, excl_factor: dict = None, topo_flip: dict = None
        ...
        if topo_flip is not None:
            ...
            print(
                "No topography info specified. In case of a lateralized "
    ...
    def create_erps(
        ...
        if save:
            evoked.save(
                self.folder_tracker(["erp", "evoked"], f"{erp_name}-ave.fif"), overwrite=True
            )
        ...
        if RT_split:
            ...
            for rt in ["fast", "slow"]:
                mask = df["RT_split"] == rt
                # create evoked objects using mne functionality and save file
                evoked_split = epochs[mask].average().apply_baseline(baseline=self.baseline)
                # if specified select time window of interest
                if time_oi is not None:
                    evoked_split = evoked_split.crop(tmin=time_oi[0], tmax=time_oi[1])
                evoked_split.save(
                    self.folder_tracker(["erp", "evoked"], f"{erp_name}_{rt}-ave.fif")
                )
        return evoked
    ...
    def generate_erp_report(self, evokeds: dict, report_name: str):
        ...
        report_name = self.folder_tracker(["erp", "report"], f"{report_name}.h5")
    ...
    def condition_erps(
        ...
        for cnd in cnds:
            ...
            evokeds[cnd] = self.create_erps(
                epochs, df, idx_c, time_oi, erp_name, RT_split, save=save
            )
    ...
    def flip_topography(
        ...
        if flip_dict is None:
            ...
            print(
                "No flip dictionary specified. Creating flip "
    ...
    def select_lateralization_idx(
        df: pd.DataFrame, pos_labels: dict, spatial_restriction: Optional[dict] = None
    ) -> np.array:
        """
        ...
        if spatial_restriction is not None:
            ...
            for key in spatial_restriction.keys():
                key_idx = np.hstack([np.where(df[key] == r)[0] for r in spatial_restriction[key]])
                if idx_r is None:
                    idx_r = key_idx
                else:
    ...
    def select_erp_window(
        ...
        if isinstance(elec_oi[0], str):
            ...
            contra_idx = np.array([channels.index(elec) for elec in elec_oi[0]])
            ipsi_idx = np.array([channels.index(elec) for elec in elec_oi[1]])
        ...
        if method == "cnd_avg":
            ...
            if isinstance(erps, dict):
                grand_mean = mne.combine_evoked(
                    [mne.combine_evoked(v, weights="equal") for (k, v) in erps.items()],
                    weights="equal",
                )
            else:
    ...
    def export_erp_metrics_to_csv(
        ...
        if not isinstance(erps, (dict, list)):
            raise ValueError("erps must be a dictionary or a list of " "evoked objects.")
        ...
        # save data
        np.savetxt(
            ERP.folder_tracker(["erp", "stats"], fname=f"{name}.csv"),
            np.stack(X).T,
            delimiter=",",
            header=",".join(headers),
            comments="",
    ...
    @staticmethod
    def group_erp(erp: list, elec_oi: list = "all") -> Tuple[np.array, mne.Evoked]:
        """
    ...
    def extract_erp_features(
        X: np.ndarray, times: np.ndarray, method: str, threshold: float = 0.5, polarity: str = "pos"
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def lateralized_erp_idx(
        erp: list, elec_oi_c: list, elec_oi_i: list
    ...
    def compare_latencies(
        ...
        for p, pair in enumerate(pairs):
            ...
            d_latency, t = ERP.jackknife_contrast(
                x1, x2, times_oi, percent_amp, cnd1_name, cnd2_name
            )
    ...
    def jackknife_contrast(
        ...
        if nr_obs != x2.shape[0]:
            raise ValueError("x1 and x2 must have the same number of " "observations")
        if nr_obs < 2:
            raise ValueError("Need at least 2 observations for " "jackknife analysis")
        ...
        try:
            d_latency = ERP.jack_latency_contrast(
                x1_, x2_, c1, c2, times, True, cnd1_name, cnd2_name
            )
        ...
        if len(D) == 0:
            raise ValueError("No valid jackknife iterations - threshold " "may be too high")

        # compute the jackknife estimate
        Sd = np.sqrt((nr_obs - 1.0) / nr_obs * np.sum([(d - np.mean(D)) ** 2 for d in np.array(D)]))

        if Sd == 0:
            raise ValueError("Standard deviation is zero - cannot" " compute t-statistic")
    ...
    def jack_latency_contrast(
        ...
        if len(idx_1_candidates) == 0:
            raise ValueError(f"No time points exceed threshold {c1:.4f} " f"for condition 1")
        idx_1 = idx_1_candidates[0]

        idx_2_candidates = np.where(x2 >= c2)[0]
        if len(idx_2_candidates) == 0:
            raise ValueError(f"No time points exceed threshold {c2:.4f} " f"for condition 2")
        ...
        if idx_1 == 0:
            ...
            if abs(denominator_1) < 1e-10:  # Very small difference
                ...
                lat_1 = (
                    times[idx_1 - 1]
                    + (times[idx_1] - times[idx_1 - 1]) * (c1 - x1[idx_1 - 1]) / denominator_1
                )
        ...
        if idx_2 == 0:
            ...
            if abs(denominator_2) < 1e-10:  # Very small difference
                ...
                lat_2 = (
                    times[idx_2 - 1]
                    + (times[idx_2] - times[idx_2 - 1]) * (c2 - x2[idx_2 - 1]) / denominator_2
                )

        d_latency = lat_2 - lat_1
        if print_output:
            print(
                f"Estimated onset latency {cnd1_name} = {lat_1:.2f}"
    ...
    def residual_eye(
        ...
        if left_info is not None:
            idx_l = self.select_lateralization_idx(beh, left_info, spatial_restriction)
        if right_info is not None:
            idx_r = self.select_lateralization_idx(beh, right_info, spatial_restriction)
        ...
        for cnd in cnds:
            ...
            # extract data
            left_wave = epochs._data[idx_c_l][:, ch_oi_idx].mean(axis=(0, 1)) * polarity_sign
            right_wave = epochs._data[idx_c_r][:, ch_oi_idx].mean(axis=(0, 1)) * polarity_sign
