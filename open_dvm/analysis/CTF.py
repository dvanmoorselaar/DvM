...
class CTF(BDM):
    ...
    def __init__(
        ...
        method: str = "Foster",
        avg_ch: bool = True,
        ctf_param: Union[str, bool] = "slopes",
        power: str = "band",
        min_freq: int = 4,
        max_freq: int = 40,
        num_frex: int = 25,
        ...
        if self.sfreq % downsample != 0:
            warnings.warn(
                f"Warning: sfreq ({self.sfreq}) is not evenly "
    ...
    def calculate_basis_set(
        self, nr_bins: int, nr_chans: int, sin_power: int = 7, delta: bool = False
    ) -> np.array:
        """
    ...
    def generate_ctf_report(
        self, ctf: dict, ctf_param, freqs: Union[str, dict], info: mne.Info, report_name: str
    ):
        """
        ...
        report_name = self.folder_tracker(["ctf", "report"], f"{report_name}.h5")
        ...
        report.save(report_name.rsplit(".", 1)[0] + ".html", overwrite=True, open_browser=False)
    ...
    def spatial_ctf(
        ...
        epochs, df = self.select_ctf_data(epochs, df, self.elec_oi, headers, excl_factor, data_type)
        ...
        if abs(actual_sfreq - self.downsample) > 1:
            print(
                f"Warning: Actual sampling frequency ({actual_sfreq}) "
        ...
        if special_loc is not None:
            ...
            if isinstance(special_loc, str):
                ...
                df.loc[test_mask, self.to_decode] = df.loc[test_mask, special_loc].values
            else:
                df.loc[test_mask, self.to_decode] = special_loc

        # based on conditions get position bins
        pos_bins, cnds, epochs, max_tr = self.select_ctf_labels(epochs, df, pos_labels, cnds)

        if GAT:
            print(
                "Creating generalization across time matrix.",
        ...
        for fr in range(nr_freqs):
            ...
            E, T = self.tfr_decomposition(epochs.copy(), freqs[fr], tois, downsample)
            ...
            for c, cnd in enumerate(train_cnds):
                for te_cnd in (test_cnds if test_cnds is not None else [None]):
                    ...
                    if fr == 0:
                        ...
                        info.update({cnd_inf: {}})
                        ctf.update({cnd_inf: {"C2_E": C2_E, "C2_T": C2_T, "W_E": W_E, "W_T": W_T}})
                        ...
                        if self.cross:
                            ...
                            if self.cross == "cross_cv":
                                ...
                                trial_limit = None
                            train_idx, test_idx = self.train_test_cross(
                                pos_bins, cnd_idx, test_idx, self.nr_iter, trial_limit
                            )
                        else:
                            train_idx, test_idx = self.train_test_split(pos_bins, cnd_idx, max_tr)
                        ...
                        if self.method == "Foster":
                            C1 = (
                                np.empty((self.nr_bins * (self.nr_folds - 1), self.nr_chans))
                                * np.nan
                            )
                        else:
                            C1 = self.basisset
                        ...
                        for itr in range(nr_itr):
                            ...
                            for bin in range(self.nr_bins):
                                if bin in test_bins:
                                    ...
                                    bin_te_E[bin] = self.extract_power(evoked_mean, freqs[fr])

                                if self.method == "Foster":
                                    for j in range(self.nr_folds - 1):
                                        evoked = self.extract_power(
                                            np.mean(E[train_idx[bin][j]], axis=0), freqs[fr]
                                        )
                            ...
                            (
                                ...
                                bin_tr_E, bin_te_E, bin_tr_T, bin_te_T, C1, GAT
                            )
        ...
        ctf_param = self.get_ctf_tuning_params(
            ctf, self.ctf_param, GAT=GAT, avg_ch=self.avg_ch, test_bins=test_bins
        )

        ctf_param.update({"info": {"times": times_oi, "freqs": freqs, "bands": bands}})

        if f_name is not None:
            if self.ctf_param:
                with open(
                    self.folder_tracker(["ctf", self.to_decode], fname=f"{ctf_name}_param.pickle"),
                    "wb",
                ) as handle:
        ...
        if self.report:
            self.generate_ctf_report(ctfs, ctf_param, freqs, info=epochs.info, report_name=ctf_name)

        # Save outputs if f_name is provided
        if f_name is not None:
            with open(
                self.folder_tracker(["ctf", self.to_decode], fname=f"{ctf_name}_ctf.pickle"), "wb"
            ) as handle:
                print("saving ctfs")
    ...
    def check_cnds_input(self, cnds: dict) -> Tuple[list, Union[list, str, None]]:
        """
    ...
    def select_bins_oi(
        self, pos_bins: np.ndarray, cnds: np.ndarray, epochs: mne.Epochs
    ...
    def set_max_trial(self, cnds: np.ndarray, pos_bins: np.ndarray, method: str) -> int:
        """
    ...
    def extract_power(
        self, x: np.ndarray, band: Union[str, list, tuple, None] = None
    ) -> np.ndarray:
        """
    ...
    def tfr_decomposition(
        self, epochs: mne.Epochs, band: Union[tuple, str], tois: slice, downsample: int
        ...
        **For broadband analysis (band = 'broadband'):**
        ...
        if isinstance(band, (list, tuple)):
            epochs.filter(
                band[0], band[1], method="iir", iir_params=dict(ftype="butterworth", order=5)
            )
    ...
    def forward_model(
        self, train_X: np.array, test_X: np.array, C1: np.array
    ...
    def forward_model_loop(
        ...
        for tr_t in range(nr_samples_tr):
            for te_t in range(nr_samples_te):
                ...
                c2_e, w_e = self.forward_model(
                    E_train[..., tr_t : tr_t + slide], E_test[..., te_t : te_t + slide], C1
                )

                # total power model fit
                c2_t, w_t = self.forward_model(
                    T_train[..., tr_t : tr_t + slide], T_test[..., te_t : te_t + slide], C1
                )
                if not GAT:
                    te_t = 0
                C2_E[tr_t, te_t] = c2_e
    ...
    def train_test_cross(
        ...
        train_idx = [np.random.choice(train_idx[train_bins == b], min_obs, False) for b in bins]
        ...
        for itr in range(nr_iter):
            ...
            if bins.size == train_bins_unique.size:
                ...
                selected_test_idx = [
                    np.random.choice(available_idx[available_test_bins == b], min_obs_test, False)
                    for b in bins
                ]
                selected_test_idx = np.array(selected_test_idx)
            else:
                # some bins missing - need to pad with zeros
                selected_test_idx = np.zeros((train_bins_unique.size, min_obs_test), dtype=int)
                bin_cnt = 0
    ...
    def train_test_split(
        self, pos_bins: np.ndarray, cnd_idx: np.ndarray, max_tr: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        train_idx = np.zeros(
            (self.nr_iter * self.nr_folds, self.nr_bins, self.nr_folds - 1, tr_per_block), dtype=int
        )
        test_idx = np.zeros((self.nr_iter * self.nr_folds, self.nr_bins, tr_per_block), dtype=int)
        ...
        for i in range(self.nr_iter):
            for bl in range(self.nr_folds):
                for bin in range(self.nr_bins):
                    ...
                    train_mask = (
                        (~np.isnan(bl_assign[i])) * (bl_assign[i] != bl) * (cnd_bins == bin)
                    )
                    # split all train data into seperate train blocks
                    train = np.array_split(trial_idx[train_mask], self.nr_folds - 1)
                    for j in range(self.nr_folds - 1):
    ...
    def set_frequencies(self, freqs: Union[str, dict]) -> Tuple[list, int]:
        ...
        if freqs == "main_param":
            if self.freq_scaling == "log":
                frex = np.logspace(np.log10(self.min_freq), np.log10(self.max_freq), self.num_frex)
    ...
    def localizer_spatial_ctf(
        self,
        pos_labels_tr: dict = "all",
        pos_labels_te: dict = "all",
        freqs: dict = "main_param",
        te_cnds: dict = None,
        te_header: str = None,
        window_oi_tr: tuple = None,
        window_oi_te: tuple = None,
        excl_factor_tr: dict = None,
        excl_factor_te: dict = None,
        downsample: int = 1,
        nr_perm: int = 0,
        GAT: bool = False,
        f_name: str = None,
        ...
        tois_tr = get_time_slice(epochs_tr.times, window_oi_tr[0], window_oi_tr[1])
        tois_te = get_time_slice(epochs_te.times, window_oi_te[0], window_oi_te[1])
        ...
        pos_bins_te, cnds, epochs_te, _ = self.select_ctf_labels(
            epochs_te, df_te, pos_labels_te, te_cnds
        )
        ...
        for fr in range(nr_freqs):
            ...
            # Time-Frequency Analysis
            E_tr, T_tr = self.tfr_decomposition(epochs_tr.copy(), freqs[fr], tois_tr, downsample)
            nr_samp_tr = 1 if avg_tr else E_tr.shape[-1]
            E_te, T_te = self.tfr_decomposition(epochs_te.copy(), freqs[fr], tois_te, downsample)
            ...
            for c, cnd in enumerate(test_cnds):
                ...
                # preallocate arrays
                C2_E = np.zeros(
                    (nr_perm, nr_freqs, nr_samp_tr, nr_samp_te, self.nr_bins, self.nr_chans)
                )
                W_E = np.zeros((nr_perm, nr_freqs, nr_samp_tr, nr_samp_te, self.nr_chans, nr_elec))
                C2_T, W_T = C2_E.copy(), W_E.copy()
                ...
                if fr == 0:
                    # update ctf dicts to keep track of output
                    info.update({cnd: {}})
                    ctf.update({cnd: {"C2_E": C2_E, "C2_T": C2_T, "W_E": W_E, "W_T": W_T}})

                    # get train and test indices
                    idx = np.arange(pos_bins_tr.size)
                    train_idx, _ = self.train_test_cross(pos_bins_tr, idx, None, nr_itr)
                    idx = cnd == cnds
                    _, test_idx = self.train_test_cross(pos_bins_te, idx, idx, nr_itr)
                    test_bins = np.unique(pos_bins_te[test_idx])

                    info[cnd]["train_idx"] = train_idx
                    info[cnd]["test_idx"] = test_idx
                    if self.method == "Foster":
                        C1 = np.empty((self.nr_bins * self.nr_folds, self.nr_chans)) * np.nan
                    else:
                        C1 = self.basisset
                ...
                for bin in range(self.nr_bins):
                    ...
                    if self.method == "Foster":
                        for j in range(self.nr_folds):
                            evoked = abs(np.mean(E_tr[train_idx[bin][j]], axis=0)) ** 2
                ...
                (
                    ...
                    ctf[cnd]["C2_T"][p, fr],
                    ctf[cnd]["W_T"][p, fr],
                ) = self.forward_model_loop(bin_tr_E, bin_te_E, bin_tr_T, bin_te_T, C1, GAT)
        ...
        if self.ctf_param:
            print("get ctf tuning params")
            ctf_param = self.get_ctf_tuning_params(ctf, self.ctf_param, GAT=GAT, avg_ch=self.avg_ch)
        else:
            ctf_param = None
    ...
    def extract_slopes(self, X: np.array) -> float:
        ...
        if self.nr_chans % 2 == 0:
            x = np.array([X[0]] + [np.mean((X[i], X[-i])) for i in range(1, steps)])
        else:
            x = np.array([np.mean((X[i], X[i + (-1 - 2 * i)])) for i in range(steps)])

        slope = np.polyfit(range(1, len(x) + 1), x, 1)[0]

        return slope

    def extract_ctf_params(
        self,
        ctfs: np.array,
        params: dict,
        signal: str,
        ...
        for freq_idx in range(nr_freqs):
            for train_sample in range(nr_samples_tr):
                for test_sample in range(nr_samples_te):
                    ...
                    if ch_idx is None:
                        params[f"{signal}_slopes"][
                            perm_idx, freq_idx, train_sample, test_sample
                        ] = slope
                    else:
                        params[f"{signal}_slopes"][
                            perm_idx, freq_idx, train_sample, test_sample, ch_idx
                        ] = slope

                    # Calculate curve fitting parameters
                    # if requested AND fitting method provided
                    if any([k for k in params.keys() if "amps" in k]) and fitting_method:
                        if fitting_method == "von_mises":
                            amplitude, baseline, conc, mean_loc, _ = self.fit_cos_to_ctf(
                                current_ctf
                            )
                            ...
                            raise ValueError(f"Unknown fitting method: " f"{fitting_method}")

                        if ch_idx is None:
                            params[f"{signal}_amps"][
                                perm_idx, freq_idx, train_sample, test_sample
                            ] = amplitude
                            params[f"{signal}_base"][
                                perm_idx, freq_idx, train_sample, test_sample
                            ] = baseline
                            params[f"{signal}_conc"][
                                perm_idx, freq_idx, train_sample, test_sample
                            ] = conc
                            params[f"{signal}_means"][
                                perm_idx, freq_idx, train_sample, test_sample
                            ] = mean_loc
                        else:
                            params[f"{signal}_amps"][
        ...
        for signal in signals:
            ...
            for perm_idx in range(nr_perm):
                ...
                if avg_ch:
                    ...
                    params = self.extract_ctf_params(
                        signal_ctfs, params, signal, perm_idx, fitting_method=fitting_method
                    )
                else:
                    ...
                    for channel_idx in range(nr_chans):
                        ...
                        if np.all(ch_ctfs == 0):
                            continue
                        params = self.extract_ctf_params(
                            ch_ctfs, params, signal, perm_idx, channel_idx, fitting_method
                        )

        return params

    def get_ctf_tuning_params(
        self,
        ctfs: dict,
        ctf_param: str = "slopes",
        GAT: bool = False,
        avg_ch: bool = True,
        test_bins: np.array = None,
    ) -> dict:
        """
        ...
        if ctf_param in ["von_mises", "gaussian"]:
            ...
            output_params += [f"{signal}_{param}" for param in tuning_params for signal in signals]
        else:
        ...
        if GAT:
            nr_perm, nr_freqs, nr_train_samples, nr_test_samples = list(data.shape)[:4]
            output = np.zeros((nr_perm, nr_freqs, nr_train_samples, nr_test_samples))
        ...
        for cnd in ctfs.keys():
            ...
            # get parameters
            ctf_param[cnd] = self.summarize_ctfs(
                ctfs[cnd],
                ctf_param[cnd],
                nr_samples,
                nr_freqs,
            ...
            def _squeeze_keep_leading(x, n_protected):
                squeeze_axes = tuple(ax for ax in range(n_protected, x.ndim) if x.shape[ax] == 1)
                return np.squeeze(x, axis=squeeze_axes) if squeeze_axes else x

            true_param = {
                k: _squeeze_keep_leading(v[0], n_protected=1) for k, v in ctf_param[cnd].items()
            }
            if nr_perm > 1:
                perm_param = {
                    f"{k}_perm": _squeeze_keep_leading(v[1:], n_protected=2)
    ...
    def fit_cos_to_ctf(
        self, ctf_data: np.array, conc_step: float = 0.1, estimate_center: bool = False
    ):
        """
        ...
        sse, baseline_estimates, amplitude_estimates = np.zeros((3, concentration_range.size))
        ...
        for mean_location in mean_locations:
            ...
            for conc_idx in range(concentration_range.size):
                ...
                predicted_response = (
                    initial_amplitude
                    * np.exp(current_concentration * (np.cos(mean_location - spatial_coords) - 1))
                    + initial_baseline
                )
                ...
                betas, _, _, _ = np.linalg.lstsq(design_matrix, ctf_data, rcond=None)
                amplitude_estimates[conc_idx], baseline_estimates[conc_idx] = betas
        ...
        final_prediction = (
            best_amplitude
            * np.exp(best_concentration * (np.cos(best_mean_location - spatial_coords) - 1))
            + best_baseline
        )
        rmse = np.sqrt(sum((final_prediction - ctf_data) ** 2) / final_prediction.size)

        return (best_amplitude, best_baseline, best_concentration, best_mean_location, rmse)
