...
class EYE(FolderStructure):
    ...
    def get_eye_data(
        ...
        if eye_files == "all":
            eye_files = glob.glob(
                self.folder_tracker(ext=["eye", "raw"], fname="sub_{}_ses_*.asc".format(sj))
            )
            beh_files = glob.glob(
                self.folder_tracker(ext=["behavioral", "raw"], fname="sub_{}_ses_*.csv".format(sj))
            )
            ...
            if len(beh_files) > len(eye_files):
                ...
                eye_sessions = [
                    int(re.search(r"ses_0?(\d+)", os.path.basename(f)).group(1)) for f in eye_files
                ]
                beh_sessions = [
                    int(re.search(r"ses_0?(\d+)", os.path.basename(f)).group(1)) for f in beh_files
                ]
                beh_files = [f for f, ses in zip(beh_files, beh_sessions) if ses in eye_sessions]
        ...
        if eye_files[0][-3:] == "tsv":
            ...
            if stop is None:
                eye = [
                    read_edf(file, start=start, trial_info=trial_info, missing=0)
                    for file in eye_files
                ]
            else:
                eye = [
                    read_edf_time_overlap(file, start=start, stop=stop, missing=0)
                    for file in eye_files
                ]
            ...
            for event in trial["events"]["msg"]:
                if start_event in event[1]:
                    ...
                    if trial["trackertime"].size > 0:
                        ...
                        target_idx = np.where(
                            (times >= trial_times[0]) & (times <= trial_times[-1])
                        )[0]

                        # Only copy data where times overlap
                        if len(target_idx) > 0:
                            x[i, target_idx] = np.interp(times[target_idx], trial_times, x_[idx])
                            y[i, target_idx] = np.interp(times[target_idx], trial_times, y_[idx])
    ...
    def set_xy(
        ...
        SD = SaccadeDetector(
            self.sfreq, screen_h=self.scr_h, viewing_dist=self.view_dist, screen_res=self.scr_res
        )
    ...
    def link_eye_to_eeg(
        ...
        eye, beh, trial_info = self.get_eye_data(
            sj="", eye_files=eye_file, beh_files=beh_file, start=start_trial, stop=stop_trial
        )
        # collect x, y data
    ...
    def calculate_angle(
        self, x: np.ndarray, y: np.ndarray, xc: Optional[float] = None, yc: Optional[float] = None
    ) -> np.ndarray:
        """
        Calculate visual angle deviation from center point.
        ...
        deg_per_px = degrees(atan2(0.5 * self.scr_h, self.view_dist)) / (0.5 * self.scr_res[1])
    ...
    @staticmethod
    def degrees_per_pixel(h: float = 30, d: float = 60, r: int = 1080) -> float:
        """
...
class SaccadeDetector(object):
    ...
    def detect_events(
        self, x: np.ndarray, y: np.ndarray, output: str = "mask"
        ...
        if output == "mask":
            sacc = np.array(
                [np.nan if isinstance(v, float) and np.isnan(v) else bool(v) for v in sacc]
            )

        return sacc

    def calc_velocity(self, x: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def estimate_thresh(
        self, V: np.ndarray, peak_thresh: float = 100, min_fix: float = 0.04
    ) -> None:
        """
        ...
        while abs(peak_thresh - old_thresh) > 1:
            ...
            if len(fix_noise) == 0:
                peak_thresh = None
                sacc_thresh = None
                print("No valid fixation periods found for " "threshold estimation")
                break
    ...
    def saccade_detection(
        self, V: np.ndarray, min_sac: float = 0.01, min_fix: float = 0.04, output: str = "mask"
    ) -> Union[int, dict]:
        """
        ...
        for seg in sac_seg:
            ...
            if seg.size <= ceil(min_sac / 6.0 * self.sfreq):
                continue
            ...
            onset = np.where(
                (V[: seg[0]] <= self.sacc_thresh) * (np.hstack((np.diff(V[: seg[0]]), 0)) >= 0)
            )[0]
            if onset.size == 0:
                continue
            sac_on = onset[-1]
            ...
            V_local = V[sac_on : int(ceil(max(0, sac_on - min_fix * self.sfreq))) : -1]
            ...
            if V_noise > self.peak_thresh:
                continue

            # detect end of saccade (without glissade)
            offset = np.where(
                (V[seg[-1] :] <= sacc_thresh) * (np.hstack((np.diff(V[seg[-1] :]), 0)) >= 0)
            )[0]
            if offset.size == 0:
                continue
            ...
            if (np.isnan(V[sac_on:sac_off]).any()) or (sac_off - sac_on) / self.sfreq < min_sac:
                continue
            ...
            if gliss_w_off:
                ...
                if (
                    ...
                    or (gliss_off - sac_off) / self.sfreq > 2 * min_fix
                ):
                    gliss_off = []
                else:
                    glissade_idx = np.array(
                        np.hstack((glissade_idx, np.arange(sac_off, gliss_off))), dtype=int
                    )
                    nr_gliss += 1
