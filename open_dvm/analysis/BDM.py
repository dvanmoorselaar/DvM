    ...
    **tfr_kwargs
        ...
        **tfr_kwargs
        ...
        if self._data_type == "tfr":
            if tfr is not None:
                ...
                raise ValueError(
                    "When data_type='tfr', must provide either "
    ...
    def data_type(self, value):
        ...
        raise AttributeError(
            "data_type cannot be changed after "
    ...
    def select_classifier(self) -> Any:
        ...
        if self.classifier == "LDA":
            ...
            if not self.scale:
                warnings.warn(
                    "SVM performance is poor without feature scaling. "
                    "Consider setting scale=True or use LDA instead.",
                    UserWarning,
                )
            clf = CalibratedClassifierCV(
                LinearSVC(random_state=self.seed, max_iter=5000, dual=False)
            )
        else:
    ...
    def classify(
        ...
        nr_perm: int = 0,
        GAT: Union[bool, Tuple[Tuple[float, float], Tuple[float, float]]] = False,
        downscale: bool = False,
        split_fact: dict = None,
        ...
        if special_col is not None:
            ...
            if special_col not in df.columns:
                raise ValueError(
                    f"special_col column '{special_col}' not found " "in the behavioral dataframe."
                )
            test_cnds = cnds[1]
            if isinstance(test_cnds, str):
                test_cnds = [test_cnds]
            test_mask = df[cnd_head].isin(test_cnds)
            df.loc[test_mask, self.to_decode] = df.loc[test_mask, special_col].values
        ...
        X, y, df, times = self.select_bdm_data(
            self.epochs.copy(), df, window_oi, excl_factor, headers, cnds
        )

        nr_labels, tr_max, tr_cnds, te_cnds = self.set_bdm_param(
            y, df, cnds, cnd_head, labels_oi, downscale
        )
        ...
        if split_fact is None:
            bdm_scores, bdm_params, bdm_info = self.classify_(
                X,
                y,
                df,
                tr_cnds,
            ...
            bdm_scores, bdm_params, bdm_info = self.iter_classify_(
                split_fact,
                X,
                y,
                df,
        ...
        if GAT:
            if isinstance(GAT, tuple):
                ...
                train_times = times[get_time_slice(times, tr_window[0], tr_window[1])]
                test_times = times[get_time_slice(times, te_window[0], te_window[1])]
            else:
                train_times = test_times = times
            bdm_scores["info"].update(
                {
        ...
        if f_name is not None:
            ...
            with open(self.folder_tracker(ext, fname=f"{bdm_name}.pickle"), "wb") as handle:
                print("saving bdm scores")
                pickle.dump(bdm_scores, handle)
            if self.output_params:
                with open(
                    self.folder_tracker(ext, fname=f"{bdm_name}_param.pickle"), "wb"
                ) as handle:
    ...
    def iter_classify_(
        ...
        for key, value in split_fact.items():
            for v in value:
                ...
                tr_max = [self.select_max_trials(df_, tr_cnds, labels_oi, cnd_head)]
    ...
    def classify_(
        ...
        for tr_cnd in tr_cnds:
            for te_cnd in (te_cnds if te_cnds is not None else [None]):
                ...
                df, cnd_idx, cnd_labels, labels, tr_max = self.get_condition_labels(
                    df, cnd_head, tr_cnd, tr_max, labels_oi, collapse
                )

                # initiate decoding arrays for current condition
                class_acc = np.empty((self.avg_runs, nr_perm, nr_freq, nr_train, nr_test)) * np.nan
                weights = np.empty((self.avg_runs, nr_perm, nr_freq, nr_train, nr_test, nr_elec))
                conf_matrix = np.empty(
                    (self.avg_runs, nr_perm, nr_freq, nr_train, nr_test, labels.size, labels.size)
                )
                    ...
                    for i, n in enumerate(tr_max):
                        ...
                        for run in range(self.avg_runs):
                            ...
                            if self.cross:
                                ...
                                train_tr, test_tr, bdm_info = self.train_test_split(
                                    cnd_idx, cnd_labels, n, bdm_info
                                )
                                Xtr, Xte, Ytr, Yte = self.train_test_select(
                                    X, y_perm, train_tr, test_tr
                                )
                            ...
                            Xte = Xte[..., te_idx]
                            Xtr_runs.append(Xtr)
                            class_acc[run, p], weights[run, p], conf_matrix[run, p] = (
                                self.cross_time_decoding(Xtr, Xte, Ytr, Yte, labels, GAT, X)
                            )
                        ...
                        if i == 0 and p == 0:
                            ...
                            W = self.set_bdm_weights(
                                W, np.concatenate(Xtr_runs, axis=0), nr_elec, nr_time
                            )

                            dec_scores = copy.copy(np.squeeze(mean_class[0]))
                            bdm_scores.update(
                                {cnd_inf: {"dec_scores": dec_scores}, "bdm_info": bdm_info}
                            )

                            bdm_params.update(
                                {cnd_inf: {"W": W, "conf_matrix": copy.copy(conf_M[0])}}
                            )

                        elif i > 0 and p == 0:
                            bdm_scores[cnd_inf]["{}-nrlabels".format(n)] = copy.copy(mean_class[0])
    ...
    def localizer_classify_(
        ...
        for cnd in cnds_te:
            ...
            # get condition indices and labels
            beh_te, cnd_idx_te, cnd_labels, labels, max_tr = self.get_condition_labels(
                beh_te, cnd_header_te, cnd, max_tr, labels_oi_te
            )
            ...
            nr_freq = 1
            class_acc = np.empty((self.avg_runs, nr_perm, nr_freq, nr_time_tr, nr_time_te)) * np.nan
            conf_matrix = (
                np.empty(
                    (
                        self.avg_runs,
            ...
            weights = (
                np.empty((self.avg_runs, nr_perm, nr_freq, nr_time_tr, nr_time_te, nr_elec))
                * np.nan
            )
            ...
            for p in range(nr_perm):
                ...
                for i, n in enumerate([1]):
                    ...
                    for run in range(self.avg_runs):
                        ...
                        Xte, _, Yte, _ = self.train_test_cross(
                            X_te, y_te_perm, cnd_idx_te, False, max_tr
                        )

                        class_acc[run, p], weights[run, p], conf_matrix[run, p] = (
                            self.cross_time_decoding(
                                Xtr, Xte, Ytr, Yte, (labels_oi_tr, labels_oi_te), GAT, Xtr
                            )
                        )
                    ...
                    if i == 0:
                        ...
                        bdm_scores.update(
                            {cnd: {"dec_scores": copy.copy(np.squeeze(mean_class[0]))}}
                        )
                        if self.output_params:
                            bdm_params.update({cnd: {"W": W, "conf_matrix": copy.copy(conf_M[0])}})

                    else:
                        bdm_scores[cnd]["{}-nrlabels".format(n)] = copy.copy(mean_class[0])
    ...
    def localizer_classify(
        self,
        te_cnds: dict = None,
        tr_window_oi: tuple = None,
        te_window_oi: tuple = None,
        tr_excl_factor: dict = None,
        te_excl_factor: dict = None,
        tr_labels_oi: Union[str, list] = "all",
        te_labels_oi: Union[str, list] = "all",
        te_header: str = None,
        ...
        X_tr, y_tr, times_tr, cnd_idx_tr = self.get_train_X(
            tr_window_oi, tr_excl_factor, tr_labels_oi, avg_window
        )
        ...
        X_te, y_te, beh_te, times_te = self.select_bdm_data(
            self.epochs[1].copy(), self.df[1].copy(), te_window_oi, te_excl_factor, [cnd_header]
        )
        ...
        bdm_scores, bdm_params = self.localizer_classify_(
            X_tr,
            y_tr,
            X_te,
            y_te,
    ...
    def sliding_window_base(self, epochs: mne.Epochs, window_size: int = 100) -> mne.Epochs:
    ...
    def get_train_X(
        self, window_oi: tuple, excl_factor: dict, labels_oi: Union[str, list], avg_window: bool
    ...
    def select_bdm_data(
        ...
        if headers and cnds is not None and cnds != ["all_data"]:
            ...
            to_exclude = [cnd for cnd in df[cnd_header].unique() if cnd not in flat_cnds]
            ...
            if to_exclude:
                ...
                print(
                    f"Automatically excluding {cnd_header}={to_exclude} "
            ...
            if self.window_size[0] == 1:
                ...
                if self._data_type == "tfr":
                    ...
                    X = mne.filter.resample(
                        X.astype(float), down=self.down_factor, npad="auto", pad="edge"
                    )
            else:
        ...
        if self.window_size[0] > 1:
            X = self.sliding_window(
                X, self.window_size[0], self.window_size[1], self.window_size[2]
            )
    ...
    def report_bdm(self, bdm_scores: dict, bdm_params: dict, cnds: list, bdm_name: str) -> None:
        """
        ...
        **Dependencies:**
        ...
        report_name = self.folder_tracker(report_path, f"{bdm_name}.html")

        # Determine if topographical plotting is possible
        plot_topographies = self.elec_oi == "all"
        if not plot_topographies:
            print("Info: Topographical plots will be skipped " "(not all electrodes used)")
        else:
            montage = None
            ...
            if self.montage is None:
                try:
                    ...
                    print(
                        "Warning: No montage found in epochs object and "
        ...
        if plot_topographies:
            ...
            for cnd in bdm_params.keys():
                if "W" in bdm_params[cnd]:
                    W = bdm_params[cnd]["W"]
                    if W.shape[0] > 1:
                        print("only first frequency will be used for " "topographical plots")
                    if W.shape[2] > 1:
                        print("only first test time point will be used for " "topographical plots")
                    ...
                    report.add_evokeds(evokeds=W_evoked, titles=f"Weights {cnd}")
    ...
    def sliding_window(
        self, X: np.array, window_size: int = 20, demean: bool = True, avg_window: bool = False
    ) -> np.array:
        """
        ...
        if self.window_size[2]:
            print("downsampling data")
            output = mne.filter.resample(output, down=self.down_factor, npad="auto", pad="edge")
    ...
    def train_test_split(
        self, idx: np.array, labels: np.array, max_tr: int, bdm_info: dict
        ...
        if steps == 0:
            suggestions = [f"(1) Reduce nr_folds from {N} to a smaller value"]

            # Add suggestion based on what user is actually using
            # Check if averaging trials
            avg_trials_val = (
                self.avg_trials[-1] if isinstance(self.avg_trials, list) else self.avg_trials
            )
            if avg_trials_val > 1:
                msg = f"(2) Reduce avg_trials from {avg_trials_val} " f"to average fewer trials"
            ...
            trials_per_class = [np.sum(labels == label) for label in unique_labels]
        ...
        if bdm_info["run_" + str(self.run_info)] == {}:
            for i, l in enumerate(np.unique(labels)):
                label_dct = {l: idx[random.sample(list(np.where(labels == l)[0]), max_tr)]}
    ...
    def train_test_select(
        self, X: np.array, Y: np.array, train_tr: np.array, test_tr: np.array
    ) -> Tuple[np.array, np.array, np.array, np.array]:
        """
        ...
        if X.ndim == 3:  # [trials × channels × time]
            Xtr = np.zeros((self.nr_folds, np.prod(train_tr.shape[-2:]), X.shape[1], X.shape[2]))
            Xte = np.zeros((self.nr_folds, np.prod(test_tr.shape[-2:]), X.shape[1], X.shape[2]))
        else:
            Xtr = np.zeros(
                (self.nr_folds, X.shape[0], np.prod(train_tr.shape[-2:]), X.shape[2], X.shape[3])
            )
            Xte = np.zeros(
                (self.nr_folds, X.shape[0], np.prod(test_tr.shape[-2:]), X.shape[2], X.shape[3])
            )
    ...
    def set_bdm_weights(
        self, W: np.ndarray, Xtr: np.ndarray, nr_elec: int, nr_time: int
    ...
    def cross_time_decoding(
        ...
        **Memory Considerations:**
        ...
        conf_matrix = np.zeros((N, nr_freq, nr_time_tr, nr_time_te, len(labels[1]), len(labels[0])))

        for n in range(N):
            print(f"\rFold {n+1} out of {N} folds " f"in run {self.run_info}", end="")
            Ytr_ = Ytr[n]
            Yte_ = Yte[n]
            ...
            for freq in range(nr_freq):
                if self.tfr is not None:
                    print(
                        f"\rFrequency {freq+1} out of {nr_freq} " f"in run {self.run_info}", end=""
                    )
                for tr_t in range(nr_time_tr):
                    ...
                    for te_t in te_time_points:
                        ...
                        if self.pca_components[0]:
                            ...
                            if self.pca_components[1] == "across":
                                ...
                                if len(X) > 0:
                                    ...
                                    if self.scale:
                                        X_ = StandardScaler(with_std=True).fit_transform(X_)
                                    pca = PCA(n_components=self.pca_components[0])
                                    pca.fit(X_)
                        ...
                        class_perf = (
                            self.compute_class_perf(scores, Yte_, np.unique(Ytr_), predict)
    ...
    def get_fake_confusion_matrix(
        self,
        y_true: np.array,
        y_pred: np.array,
        row_labels: np.array = None,
        col_labels: np.array = None,
    ) -> np.array:
    ...
    def compute_class_perf(
        self, scores: np.ndarray, true_labels: np.ndarray, label_order: list, predict: np.ndarray
    ) -> float:
        """
        ...
        **Balanced Accuracy Calculation:**
        ...
        if self.metric == "auc":
            ...
            for i, comp in enumerate(pairs):
                ...
                pair_idx = np.logical_or(true_labels == comp[0], true_labels == comp[1])
            ...
            for label in unique_labels:
                ...
                if np.sum(class_mask) > 0:  # Avoid division by zero
                    class_correct = np.sum(predict[class_mask] == true_labels[class_mask])
    ...
    def select_max_trials(
        self,
        df: pd.DataFrame,
        cnds: list,
        bdm_labels: Union[str, list] = "all",
        cnds_header: str = "condition",
    ) -> int:
        """
        ...
        if max_trials == 0:
            print(
                "At least one condition does not contain sufficient " "info for current nr of folds"
            )
