...
class TFR(FolderStructure):
    ...
    def _get_wavelet_params_hash(self):
        ...
        params = (
            ...
            self.cycle_range,
            self.freq_scaling,
            self.normalize_wavelets,
            self.epochs.info["sfreq"],
            self.epochs.times.size,
        )
    ...
    def _ensure_wavelets(self):
        ...
        if self.method == "wavelet":
            if (
                not hasattr(self, "wavelets")
                or self.wavelets is None
                or not hasattr(self, "frex")
                or self.frex is None
                or self._wavelet_params_hash != current_hash
            ):
            ...
            if (
                not hasattr(self, "freq_bands")
                or self.freq_bands is None
                or not hasattr(self, "frex")
                or self.frex is None
                or self._wavelet_params_hash != current_hash
            ):
    ...
    def select_tfr_data(
        ...
        if topo_flip is not None:
            ...
            print(
                "No topography info specified. For lateralization analysis,"
    ...
    def create_freq_bands(self) -> list:
        ...
        if self.freq_scaling == "log":
            center_freqs = np.logspace(
                np.log10(self.min_freq), np.log10(self.max_freq), self.num_frex
            )
        else:  # linear
            center_freqs = np.linspace(self.min_freq, self.max_freq, self.num_frex)
    ...
    def create_morlet(
        ...
        if freq_scaling == "log":
            ...
            s = np.logspace(np.log10(cycle_range[0]), np.log10(cycle_range[1]), num_frex)
            s /= 2 * math.pi * frex
        ...
        for fi in range(num_frex):
            wavelet = np.exp(2j * math.pi * frex[fi] * t) * np.exp(-(t**2) / (2 * s[fi] ** 2))
    ...
    def generate_tfr_report(self, tfr: dict, info: mne.Info, report_name: str):
        """
        ...
        **Interactive Features:**
        ...
        report_name = self.folder_tracker(["tfr", "report"], f"{report_name}.h5")
        ...
        report.save(report_name.rsplit(".", 1)[0] + ".html", open_browser=False, overwrite=True)
    ...
    def compute_tfrs(
        ...
        if self.power == "induced":
            if cnd_idx is None:
                raise ValueError("cnd_idx must be provided when " "power is 'induced'")

        if self.power == "induced":
            # subtract condition specific evoked from each trial
            print(
                "Calculating induced power: subtracting condition specific "
    ...
    def condition_tfrs(
        ...
        **File Organization:**
        ...
        for c, cnd in enumerate(cnds):
            ...
            if self.power == "induced":
                print("Calculating induced power (i.e., subtracting evoked " "response)")
                evoked = epochs[idx_c].average()
                tfr_epochs = epochs[idx_c].subtract_evoked(evoked)
            elif self.power == "evoked":
                print("Calculating evoked power (i.e., average of " "epochs)")
            ...
            if self.baseline is not None:
                ...
                if self.base_method == "trial_spec":
                    base[cnd] = np.mean(base_power, axis=-1)
                else:  # cnd_avg or cnd_spec
                    base[cnd] = np.mean(base_power, axis=(0, 3))
            else:
                base = {}
            # populate tf
    ...
    def tfr_loop(self, epochs: mne.Epochs) -> np.array:
        ...
        for ch_idx in range(nr_ch):
            print(f"Decomposing channel {ch_idx+1} out of {nr_ch} channels", end="\r")
            ...
            if self.method == "wavelet":
                ...
                for f, (l_freq, h_freq) in enumerate(self.freq_bands):
                    ...
                    x_filt = mne.filter.filter_data(
                        x, s_freq, l_freq, h_freq, method="fir", phase="zero-double", verbose=False
                    )
    ...
    def save_to_mne_format(self, tfr: dict, epochs: mne.Epochs, tfr_name: str, save: bool = False):
        """
        ...
        for cnd in tfr["power"].keys():
            ...
            if save:
                save_path = self.folder_tracker(["tfr", self.method], f"{tfr_name}_{cnd}-tfr.h5")
        ...
        **Output Format:**
        ...
        for cnd in cnds:
            ...
            if method is None or (needs_base and not base):
                ...
                print(
                    "For normalization procedure it is assumed that it is as"
                ...
                tfr["power"][cnd], info = self.normalize_power(avg_power, list(tfr["ch_names"]))
    ...
    def normalize_power(self, avg_power: np.ndarray, ch_names: list) -> Tuple[np.ndarray, dict]:
        """
