...
def find_raw_files(folder: str, sj: int, session: int, 
    ...
    - sub_002_ses_001.bdf (three digit)
    ...
    ----------
    folder : str
        Root folder to search for files
    sj : int
        Subject number
    session : int
        Session number
    run : int, optional
        Run number. If None, only returns files without run component.
    ext : str, default='bdf'
    ...
    for fpath in matching_files:
        ...
        if match:
            ...
            if file_sj == sj and file_ses == session:
                ...
                if run is not None:
                    if file_run == run:
                        # Exact match: file has matching run
                        candidates.append(fpath)
                    elif flexible_run and file_run is None:
                        # Flexible mode: accept file without run suffix
                        candidates.append(fpath)
